...
vignette: >
  %\VignetteIndexEntry{Indiferent Zone approach for binomial outcomes}
...
`wcs_power_best_binomial()` search for the probability in the best group that
leads to the lowest power give a pre-specified indifferent-zone threshold 
...
weights of `c(0.4, 0.4, 0.2)`. Weights are scaled to sum to the number of
outcomes.

The functions are flexible and allow to specify, for each outcome, the event
